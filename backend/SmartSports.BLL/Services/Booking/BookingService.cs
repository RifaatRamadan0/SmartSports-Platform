...
namespace SmartSports.BLL.Services.Booking;
// Imported inside the namespace so entity types (e.g. Booking) resolve before the
...
public class BookingService : IBookingService
{
    ...
    public async Task CancelBookingAsync(int userId, int bookingId, string? cancellationReason)
    {
        ...
        var bookingStartUtc = PitchTime.ToUtc(booking.BookingDate, booking.StartTime);
        if (bookingStartUtc <= DateTime.UtcNow.AddHours(1))
